'use client'
import { FirstSentinelBrokerDocumentAQSEListingProposalOverviewImage, ReferenceDocImage } from '@/assets'
import { CommonBackIcon } from '@/assets/common-assets'
import { CheckboxForm } from '@/components/checkbox-form'
import { Stack, Typography, Box, Tooltip, Link, Button, Dialog, Modal, } from '@mui/material'
    ...
    <>
      <Stack sx={{ px: 5, py: 2 }}>
        ...
        <Stack direction={{md:"row",xs:"column"}} justifyContent="space-between" alignItems="center">
          ...
          >
            <Image
              ...
              width={40}
              height={40}
              onClick={() => { router.replace("/aqse-documents") }}
            ...
            >
              First sentinel broker document AQSE listing proposal overview       </Typography>
          </Stack>
          <Button variant='text' sx={{ textDecoration: "underline", color: "#0246BC", fontWeight: "semi-bold", fontSize: 40 }} onClick={() => {
            window.open(
        ...
        <Stack direction={"row"} justifyContent="space-between" alignItems="center">
          ...
          <Button variant="text" sx={{ fontWeight: "semi-bold", fontSize: {md:"2.5rem",xs:"1rem"}, color: "#5A5867" }} endIcon={<ExpandCircleDownOutlinedIcon sx={{ width: {md:40,xs:20}, height: {md:40,xs:20} }} />} onClick={() => { setOpen(true) }}>Key Terms</Button>
        </Stack>
        <Box
          sx={{
            position: 'relative',
      ...
      <Modal onClose={() => { setOpen(false) }} open={open} sx={{ height: "fit-content", width: "fit-content", top: {md:180,xs:250}, left: {md:1000,xs:0}, border: "dashed",  }}>
        <Box sx={{ p: 2, background: "white" }}>
          <Image src={ReferenceDocImage} alt="Reference Image" style={{width:"100%",height:"100%"}}/>
        </Box>
      </Modal>
    </>
  )
